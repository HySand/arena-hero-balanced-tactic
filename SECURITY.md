...
Run `verify.cmd` before every commit or release. It invokes `security_check.ps1`, which scans Git upload candidates for common tokens, private keys, personal email addresses, local usernames, computer names, absolute home paths, private IP addresses, sensitive exports, and embedded remote credentials. Findings include only the file, line number, and rule name; matched values are never printed.
