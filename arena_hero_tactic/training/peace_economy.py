...
from ..configuration.strategy import load_strategy_config, save_strategy_config
from ..runtime.paths import CONFIG_FILE, ECONOMY_TELEMETRY_FILE, STATE_FILE
from .experiment import load_records as load_telemetry_records
...
) -> tuple[list[Mapping[str, Any]], list[int | float]]:
    """Load repeatable training input from telemetry or legacy runtime state."""
    ...
    if telemetry_path is not None:
        ...
        if measurements_only:
            records = [
                record
                for record in records
                if bool(record.get("measurement_eligible"))
            ]
        return records, []

    state = json.loads(state_path.read_text(encoding="utf-8"))
    history = state.get("economy_history", ())
    cycle_durations = state.get("cycle_durations", ())
    if not isinstance(history, list) or not isinstance(cycle_durations, list):
        raise ValueError("training state has invalid economy history")
    return history, cycle_durations
...
def main() -> None:
    ...
    source.add_argument(
        "--telemetry",
        nargs="?",
        const=ECONOMY_TELEMETRY_FILE,
        type=Path,
        help="train from JSONL telemetry (default path when value is omitted)",
    )
    source.add_argument(
        "--state",
        type=Path,
        help="train from a legacy runtime state snapshot",
    )
    ...
    history, cycle_durations = load_training_data(
        telemetry_path=args.telemetry,
        state_path=args.state or STATE_FILE,
        candidate_id=args.candidate_id,
        measurements_only=args.measurements_only,
    )
    result = train_peace_economy(history, cycle_durations)
    output: dict[str, Any] = {
        "input": "telemetry" if args.telemetry is not None else "state",
