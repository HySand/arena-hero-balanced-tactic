...
DASHBOARD_STATE_FILE = Path(__file__).with_name(
...
def _neighbors(position: Position) -> tuple[Position, ...]:
    return tuple(_step(position, direction) for direction in DIRECTION_ORDER)
...
def _cell_is_visible(turn: Turn, target: Position, obstacles: set[Position]) -> bool:
    return any(
        _distance(position, target) <= radius
        and _has_line_of_sight(position, target, obstacles)
        for position, radius in _viewers(turn)
    )
...
class TacticMemory:
    ...
    def _observe_threats(self, turn: Turn) -> None:
        """Remember where enemies were seen and how healthy our units are."""
...
def _enemy_priority(enemy: object, core_position: Position) -> tuple[int, int, int, bytes]:
    kind_priority = 0 if enemy.kind == "CORE" else 1
    return (
...
def _threatens_workers(turn: Turn, enemy: object) -> bool:
    """True when an enemy is close enough to any worker to warrant a response."""
    if not SAFETY_ENABLED or _enemy_danger_radius(enemy) is None:
        return False
    return any(
        _distance(enemy.position, worker.position) <= WORKER_GUARD_RADIUS
...
def _control_workers(
    ...
    # ---- phase 3: miners first, then a bounded set of sector scouts --------
    idle_workers = max(0, len(seekers) - len(assignment))
    ...
    for worker_index, worker in enumerate(seekers):
        ...
        if scouts_sent >= scout_budget or core_position is None:
            # Surplus idle workers stage around the Core instead of clustering
            # on one cell or following the same exploration direction.
            _clear_scout_target(memory, worker.id)
            if core_position is not None:
                movement.move_toward(
                    worker,
...
def _control_vanguards(
    ...
    all_enemies = sorted(
        ...
        key=lambda enemy: _enemy_priority(enemy, turn.core.position),
    )
    threats = [
        enemy
        for enemy in all_enemies
        if _distance(enemy.position, turn.core.position) <= DEFENSE_RADIUS
        or _threatens_workers(turn, enemy)
    ]
    scout_radius = _combat_scout_radius(turn, memory, config)
    offense_ready = (not config.pacing.enabled) or _offense_ready(
        turn, memory, config
    )
    offense_targets = [
        enemy
    ...
    frontier = [
        ...
        if _distance(cell, turn.core.position) <= scout_radius
    ]
    ...
    for index, vanguard in enumerate(
        ...
        core_moving = turn.core.view.state is CoreState.MOVING
        adjacent = [
            enemy
            for enemy in all_enemies
        ...
        if adjacent:
            ...
            _clear_combat_target(memory, vanguard.id)
            movement.move_toward(vanguard, threats[0].position, avoid_danger=core_moving)
        elif core_moving:
            _clear_combat_target(memory, vanguard.id)
            movement.move_toward(
                vanguard,
                _staging_waypoint(turn.core.position, index),
                avoid_danger=True,
            )
        ...
        elif offense_targets:
            _clear_combat_target(memory, vanguard.id)
            target = min(
                offense_targets,
                key=lambda enemy: (
                    _distance(vanguard.position, enemy.position),
                    _enemy_priority(enemy, turn.core.position),
                ),
            )
            movement.move_toward(vanguard, target.position)
...
def _control_rangers(
    ...
    defense_threats = [
        enemy
        for enemy in all_enemies
        if _distance(enemy.position, turn.core.position) <= DEFENSE_RADIUS + 2
        or _threatens_workers(turn, enemy)
    ]
    offense_ready = (not config.pacing.enabled) or _offense_ready(
        turn, memory, config
    ...
    offense_targets = [
        ...
        <= (
            config.pacing.offense_radius
            if config.pacing.enabled
            else _combat_scout_radius(turn, memory, config)
        )
    ]
    rangers = sorted(
        turn.rangers,
    ...
    guard_count = (
        len(rangers)
        if assessment.safety_first
        else min(
            len(rangers),
            (
                config.rangers.guard_numerator * len(rangers)
                + config.rangers.guard_denominator
                - 1
            )
            // config.rangers.guard_denominator,
        )
    )
    guard_ids = {ranger.id for ranger in rangers[:guard_count]}
    core_moving = turn.core.view.state is CoreState.MOVING
    if core_moving:
        guard_ids = {ranger.id for ranger in rangers}
    frontier = [
        cell
        for cell in _frontier_targets(memory, movement.pathfinder)
        if _distance(cell, turn.core.position)
        <= _combat_scout_radius(turn, memory, config)
    ]
    claimed_targets: set[Position] = set()
    claimed_sectors: set[int] = set()
    ...
    for index, ranger in enumerate(rangers):
        ...
        mission_enemies = (
            defense_threats
            if guarding or not offense_ready
            else offense_targets
        )
        targets = [
            enemy
            for enemy in all_enemies
            if _distance(ranger.position, enemy.position) in {1, 2, 3}
            if _line_is_clear(ranger.position, enemy.position, movement.obstacles)
        ]
        if targets:
            _clear_combat_target(memory, ranger.id)
            ranger.shoot(targets[0])
        elif mission_enemies:
            _clear_combat_target(memory, ranger.id)
            movement.move_toward(ranger, mission_enemies[0].position, avoid_danger=core_moving)
        elif guarding:
            _clear_combat_target(memory, ranger.id)
            movement.move_toward(
                ranger,
...
def _choose_spawn(
    ...
    worker_cost = UNIT_COSTS[UnitType.WORKER]
    ...
    if assessment.safety_first and defenders < defender_target:
        for unit_type in (UnitType.VANGUARD, UnitType.RANGER):
            if available_resources >= UNIT_COSTS[unit_type]:
                return unit_type
    ...
    if config.production.enabled:
        plan_reserve = max(reserve, config.production.reserve_resources)
        for step in config.production.order:
            ...
            if counts[unit_type] >= target:
                continue
            if available_resources >= UNIT_COSTS[unit_type] + plan_reserve:
                return unit_type
            return None
    ...
    if _offense_economy_ready(turn, memory or TacticMemory(), config):
        ...
        for unit_type, target in offense_targets:
            if counts[unit_type] >= target:
                continue
            if available_resources >= UNIT_COSTS[unit_type] + offense_reserve:
                return unit_type
            return None
    ...
    candidates = [
        ...
        and available_resources >= UNIT_COSTS[unit_type] + reserve
    ]
    if not candidates:
        return None
